...
const cartModal = renderModalWindow('body', 'cart', 'Cart');

renderLoginBtn('.user-action', 'login');
renderAddProductBtn('.user-action', 'add-product');
renderLoginForm(modalWindow);
...
btnCart.onclick = function() {
  if (!isCart) {
    ...
    fetch(urls.cart)
      ...
      .then(data => {
        cart = data;
        renderCart(products, cartModal, cart, 'body');
      });
...
// FUNCTIONS -------------------------------------
function renderLoginForm(parentEl) {
...
function loginFormSubmitHandler(event) {
  ...
  const loginInput = doc.querySelector('[name="login"]');
  const pwdInput = doc.querySelector('[name="pwd"]');
  ...
  fetch(urls.users)
    .then(res => res.json())
    .then(users => {
      const authenticatedUser = users.find(user => user.login === login && user.pwd === password);
      if (authenticatedUser) {
        loginUser(authenticatedUser.role);
      } else {
        alert('Invalid login credentials. Please try again.');
      }
    });

  closeModalWindow('modal-window');
}
